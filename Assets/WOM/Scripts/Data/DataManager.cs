...
public class DataManager : MonoBehaviour
{
    ...
    public IEnumerator SetDatas()
    {
        ...
        // SET UNION GAMBLE DATA ( ÎΩëÍ∏∞ ?ç∞?ù¥?Ñ∞ , ÎΩëÍ∏∞ Í∑∏Î†à?ù¥?ìú ?ç∞?ù¥?Ñ∞ )
        SetGamleData();

        // ADD SALE STAT LIST
        AddSaleStatElements();

        // SET SALE STAT DATA ( ?åêÎß? Í∞??ä•?ïú ?ä§?ÉØ ?ç∞?ù¥?Ñ∞ )
        SetSaleStatDatas();
    ...
    void SetQuestData()
    {
        ...
        newUserDatas = GetData<NewUserDatas>(SheetDataType.newUserData);
    }

    public MineAndFactoryBuildingData GetBuildDataMineByLevel(int level)
    {
        //any Î•? ?ù¥?ö©?ïú ?òà?ô∏Ï≤òÎ¶¨
    ...
    DungeonMonsterData GetDungeonMonsterDataByLevel(DungeonMonsterDatas monsterDatas, int level)
    {
        ...
        {
            // ?ç∞?ù¥?Ñ∞ ?óÜ?ùå
            Debug.Log("Îß•ÏãúÎ©? ?†àÎ≤? ?èÑ?ã¨ ?ïò????äµ?ãà?ã§.");
